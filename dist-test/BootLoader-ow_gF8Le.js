import { br as e } from "./src-C-qx_Mx3.js";
import { o as t, r as n } from "./cws-bridge-BVaPzxvD.js";
import { n as r } from "./SettingsTypes-DAv90T5n.js";
import { c as i, p as a } from "./Settings-BAxgoWK0.js";
import { t as o } from "./hub-socket-boot-BX1DkXVm.js";
import "./state-storage-BJnUdhO3.js";
import { t as s } from "./Theme-CdcoWri7.js";
import { a as c, c as l, i as u, o as d, r as f, s as p, t as m } from "./registry-DLZ4JdC3.js";
import { i as h } from "./channel-mixin-Bfoy3DA8.js";
import "./views-AD17wavT.js";
...
var C = {
	"vl-advanced": {
		...
		initFn: async () => {
			try {
				await S("advanced"), console.log("[Styles] Veela Advanced loaded");
			} catch {}
		}
	},
	"vl-basic": {
		...
		initFn: async () => {
			try {
				await S("basic"), console.log("[Styles] Veela Basic Styles loaded");
			} catch (e) {
	...
	"vl-beercss": {
		...
		initFn: async () => {
			try {
				await S("beercss"), console.log("[Styles] Veela BeerCSS loaded");
			} catch (e) {
	...
	"vl-core": {
		...
		initFn: async () => {
			try {
				await S("core"), console.log("[Styles] Veela Core loaded");
			} catch (e) {
...
async function T(e) {
	let t = C[e] || C["vl-basic"];
	if (!t) throw Error(`Unknown style system: ${e}`);
	if (w === e) {
		console.log(`[Styles] Style system '${e}' already loaded`);
		return;
	}
	console.log(`[Styles] Loading style system: ${t.name}`), t.initFn && await t.initFn(), w = e, console.log(`[Styles] Style system ${t.name} loaded`);
}
//#endregion
//#region ../../projects/subsystem/src/boot/BootLoader.ts
var E = (e) => e === "faint" ? "tabbed" : e === "base" ? "immersive" : e, D = {
	raw: {
	...
	async boot(e, c) {
		console.log("[BootLoader] Starting boot sequence:", c);
		try {
			if (this.shellInstance) try {
				this.implicitBridgeCleanup?.(), this.implicitBridgeCleanup = null, m.unload(this.shellInstance.id);
			} catch (e) {
			...
			v(), n().catch(() => {});
			try {
				let { initFrontendDebugCapture: e } = await import("./frontend-debug-capture-Boq_GZlO.js").then((e) => e.t);
				e();
			} catch {}
			let l = await a().catch((e) => (console.warn("[BootLoader] Failed to load settings:", e), null)), u = l;
			if (t()) {
				let e = await i().catch(() => null);
				e && (u = e);
			}
			u && o(u).catch(() => void 0), s(u ?? r);
			try {
				let { initIngressPWA: e } = await import("./sw-handling-BhGPvtxU.js");
				await e();
			} catch (e) {
				console.warn("[BootLoader] Share-target / service worker ingress failed (non-fatal):", e);
			}
			await this.loadStyles(c.styleSystem);
			let d = this.resolveThemeFromSettings(l), f = await this.loadShell(c.shell, e);
			return f.setTheme(c.theme || d), await f.mount(e), this.implicitBridgeCleanup?.(), this.implicitBridgeCleanup = p(), c.channels && c.channels.length > 0 && await this.initChannels(c.channels, c.channelPriorityId), c.skipInitialNavigate ? this.dismissShellLoadingSpinner(f) : await f.navigate(c.defaultView), this.setPhase("ready"), c.rememberChoice && this.savePreferences(c), console.log("[BootLoader] Boot complete"), f;
		} catch (e) {
	...
	loadPreferences() {
		try {
			...
			let e = E(localStorage.getItem("rs-boot-shell") || "minimal");
			return {
	...
	clearPreferences() {
		try {
			localStorage.removeItem("rs-boot-style"), localStorage.removeItem("rs-boot-shell"), localStorage.removeItem("rs-boot-view"), localStorage.removeItem("rs-boot-remember"), localStorage.removeItem(l);
		} catch {}
	}
}.getInstance();
//#endregion
export { O as bootLoader, O as default };
