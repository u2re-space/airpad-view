...
var s = (() => {
	...
	...I,
	link: o(/^!?\[(label)\]\((.*?)\)/).replace("label", M).getRegex(),
	reflink: o(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", M).getRegex()
}, L = {
	...I,
	...
	delLDelim: xe,
	delRDelim: Se,
	url: o(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", F).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
...
function V(e, t) {
	if (t) {
		if (c.escapeTest.test(e)) return e.replace(c.escapeReplace, B);
	} else if (c.escapeTestNoEncode.test(e)) return e.replace(c.escapeReplaceNoEncode, B);
	return e;
}
function H(e) {
	try {
...
${c}` : c;
	...
	tokenizer;
	constructor(e) {
		this.tokens = [], this.tokens.links = Object.create(null), this.options = e || r, this.options.tokenizer = this.options.tokenizer || new K(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
			inLink: !1,
			inRawBlock: !1,
			top: !0
		};
...
`;
	...
	textRenderer;
	constructor(e) {
		this.options = e || r, this.options.renderer = this.options.renderer || new J(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new Y();
	}
	static parse(t, n) {
...
}, $.parseInline = Q.parseInline, $.Parser = X, $.parser = X.parse, $.Renderer = J, $.TextRenderer = Y, $.Lexer = q, $.lexer = q.lex, $.Tokenizer = K, $.Hooks = Z, $.parse = $;
var Ie = $.options, Le = $.setOptions, Re = $.use, ze = $.walkTokens, Be = $.parseInline, Ve = $, He = X.parse, Ue = q.lex;
//#endregion
export { t as n, $ as t };
