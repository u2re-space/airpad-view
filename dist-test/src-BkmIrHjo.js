import { $ as e, At as t, Dr as n, Dt as r, Er as i, J as a, Mr as o, Mt as s, Ot as c, Pt as l, Q as u, Tr as d, Zn as f, at as p, br as m, dt as h, er as g, ft as _, ir as v, it as y, kr as b, lr as x, lt as S, nt as C, pt as ee, rr as w, sr as T, tr as E, tt as te, wn as ne, wr as D, yr as re } from "./src-C-qx_Mx3.js";
import { c as ie } from "./src-CFwTR5EZ.js";
import { t as O } from "./AssignObject-Bp8BNqEZ.js";
import { i as k } from "./channel-actions-k77dxast.js";
import "./src-Be-NRxK4.js";
import { r as ae, t as oe } from "./ContextMenu-B7lllIkC.js";
...
[
	...
	if (typeof CSS < "u" && !A.has(e.name)) try {
		CSS.registerProperty?.(e), A.add(e.name);
	} catch {}
});
var se = async (e, t, n = !1) => {
	let r = [t?.layout?.columns || t?.layout?.[0] || 4, t?.layout?.rows || t?.layout?.[1] || 8], { item: i, list: a, items: o } = t;
	return await new Promise((e) => queueMicrotask(() => e(!0))), E?.(i, (t, n) => {
		...
		if (i && !i?.cell && (i.cell = O(v([0, 0]))), n === "cell") {
			let t = ie(i?.cell || [0, 0], c);
			t[0] !== i?.cell?.[0] && i?.cell && (i.cell[0] = t?.[0]), t[1] !== i?.cell?.[1] && i?.cell && (i.cell[1] = t?.[1]), D(e, "--p-cell-x", t?.[0]), D(e, "--p-cell-y", t?.[1]), D(e, "--cell-x", t?.[0]), D(e, "--cell-y", t?.[1]);
		}
	});
}, ce = async (e, { layout: t, dragging: n, currentCell: i, syncDragStyles: a }, { item: o, items: u, list: f }) => {
	let p = null, m = (t, n) => {
	...
	let g = () => {
		...
		return s < i.left || s > i.right || c < i.top || c > i.bottom ? null : d(t, [s, c], r, "floor");
	}, v = (e, t) => {
		...
		if (D(e, "--p-cell-x", n[0]), D(e, "--p-cell-y", n[1]), D(e, "--cell-x", n[0]), D(e, "--cell-y", n[1]), e.setAttribute("data-dragging", ""), t && Array.isArray(t)) try {
			t[0].value = 0, t[1].value = 0;
		} catch {}
		return D(e, "--drag-settle-ms", "0ms"), a?.(!0), m("onGrab", "source"), [0, 0];
	};
	return c((t) => new r(e, {
		handler: "*",
		anyPointer: !0,
		...
		return requestAnimationFrame(async () => {
			D(e, "--p-cell-x", i?.[0]?.value ?? o?.cell?.[0] ?? 0), D(e, "--p-cell-y", i?.[1]?.value ?? o?.cell?.[1] ?? 0), n && (D(e, "--cell-x", n[0]), D(e, "--cell-y", n[1]));
			let t = e.parentElement;
			if (t) {
				...
				D(e, "--cs-sw-unit-x", `${s / l}px`), D(e, "--cs-sw-unit-y", `${c / u}px`);
			}
			a?.(!0), D(e, "--drag-settle-ms", "240ms"), D(e, "will-change", "transform"), m("onRelax", "destination"), e.style.removeProperty("--cs-transition-c"), e.style.removeProperty("--cs-transition-r");
			...
			requestAnimationFrame(() => {
				D(e, "will-change", "auto"), b(), a?.(!0), n && (y(n), D(e, "--p-cell-x", n[0]), D(e, "--p-cell-y", n[1]), D(e, "--cell-x", n[0]), D(e, "--cell-y", n[1])), l?.cancel?.(), e.removeAttribute("data-dragging"), m("onPlace", "destination"), p = setTimeout(() => {
					m("onHover", "source"), p = null;
...
var le = (e, t) => {
	se(e, t, !0);
	let { item: n, items: r, list: i } = t, a = [t?.layout?.columns || t?.layout?.[0] || 4, t?.layout?.rows || t?.layout?.[1] || 8], o = !!t?.immediateDragStyles, s = [w(0, b()), w(0, b())], c = [w(n?.cell?.[0] || 0), w(n?.cell?.[1] || 0)];
	D(e, "--cell-x", c?.[0]?.value || 0), D(e, "--cell-y", c?.[1]?.value || 0);
	let l = () => {
		let t = s?.[0]?.value || 0, n = s?.[1]?.value || 0;
		D(e, "--drag-x", t), D(e, "--cs-drag-x", `${t}px`), D(e, "--drag-y", n), D(e, "--cs-drag-y", `${n}px`);
	}, u = null, d = (e = !1) => {
	...
	return E([s[0], "value"], (e, t) => {
		t === "value" && f();
	}), E([s[1], "value"], (e, t) => {
		t === "value" && f();
	}), d(!0), E([c[0], "value"], (t, r) => {
		r === "value" && n.cell != null && t != null && D(e, "--cell-x", (n.cell[0] = t) || 0);
	}), E([c[1], "value"], (t, r) => {
		r === "value" && n.cell != null && t != null && D(e, "--cell-y", (n.cell[1] = t) || 0);
	}), e.dataset.dragResetBound || (e.dataset.dragResetBound = "1", e.addEventListener("m-dragstart", () => {
		D(e, "--drag-settle-ms", "0ms"), e.style.removeProperty("--cs-transition-c"), e.style.removeProperty("--cs-transition-r");
	})), ce(e, {
		layout: a,
		currentCell: c,
		dragging: s,
...
}, ue = (e) => e === "open-view", de = (e) => e === "open-link", fe = (e, t, n, r) => {
	if (e) {
		if (e.innerHTML = "", r) {
	...
	let { mode: t, initial: n, actionOptions: r, viewOptions: i, onSave: a, onDelete: o, isViewAction: s = ue, isHrefAction: c = de, registerForBackNavigation: l = !1 } = e, u = document.createElement("div");
    ...
    `;
	let d = u.querySelector("form"), p = d?.querySelector("input[name=\"label\"]"), m = d?.querySelector("input[name=\"icon\"]"), h = d?.querySelector("select[name=\"shape\"]"), g = d?.querySelector("select[name=\"action\"]"), _ = d?.querySelector("select[name=\"view\"]"), v = d?.querySelector("input[name=\"href\"]"), y = d?.querySelector("textarea[name=\"description\"]"), b = d?.querySelector("[data-field=\"view\"]"), x = d?.querySelector("[data-field=\"href\"]");
	p && (p.value = String(n.label || "New shortcut")), m && (m.value = String(n.icon || "sparkle"));
	let S = String(n.shape || "squircle").toLowerCase();
	h && (h.value = [
		"circle",
		"square",
		"squircle"
	].includes(S) ? S : "squircle"), v && (v.value = String(n.href || "")), y && (y.value = String(n.description || "")), fe(g, r, String(n.action || "")), fe(_, i, String(n.view || ""), {
		value: "",
		label: "Choose view"
	});
	let C = () => {
		...
		ee?.(), ee = null, document.removeEventListener("keydown", w), u.remove();
	};
	g?.addEventListener("change", C), C(), document.addEventListener("keydown", w), u.addEventListener("click", (e) => {
		e.target === u && T();
		...
		e.preventDefault(), a({
			label: String(p?.value || "").trim() || "Item",
...
}, _e = (e) => he(ge[e.toLowerCase()], 6), ve = /^#?([0-9a-f]{8}|[0-9a-f]{6}|[0-9a-f]{4}|[0-9a-f]{3})$/i, ye = (e) => {
	let t;
	return (t = e.match(ve)) ? he(parseInt(t[1], 16), t[1].length) : void 0;
}, j = "([+-]?\\d*\\.?\\d+(?:[eE][+-]?\\d+)?)";
`${j}`;
var be = `${j}%`;
`${j}`;
var xe = `(?:${j}%|${j})`, Se = `(?:${j}%|${j}|none)`, Ce = `(?:${j}(deg|grad|rad|turn)|${j})`;
`${j}${j}`;
var we = "\\s*,\\s*";
RegExp("^" + Se + "$");
//#endregion
//#region ../../../node_modules/culori/src/rgb/parseRgbLegacy.js
var Te = RegExp(`^rgba?\\(\\s*${j}${we}${j}${we}${j}\\s*(?:,\\s*${xe}\\s*)?\\)$`), Ee = RegExp(`^rgba?\\(\\s*${be}${we}${be}${we}${be}\\s*(?:,\\s*${xe}\\s*)?\\)$`), De = (e) => {
	let t = { mode: "rgb" }, n;
	if (n = e.match(Te)) n[1] !== void 0 && (t.r = n[1] / 255), n[2] !== void 0 && (t.g = n[2] / 255), n[3] !== void 0 && (t.b = n[3] / 255);
	else if (n = e.match(Ee)) n[1] !== void 0 && (t.r = n[1] / 100), n[2] !== void 0 && (t.g = n[2] / 100), n[3] !== void 0 && (t.b = n[3] / 100);
	else return;
	return n[4] === void 0 ? n[5] !== void 0 && (t.alpha = Math.max(0, Math.min(1, +n[5]))) : t.alpha = Math.max(0, Math.min(1, n[4] / 100)), t;
}, Oe = (e, t) => e === void 0 ? void 0 : typeof e == "object" ? e.mode === void 0 ? t ? {
	...e,
	mode: t
} : void 0 : e : Ye(e), ke = (e = "rgb") => (t) => (t = Oe(t, e)) === void 0 ? void 0 : t.mode === e ? t : M[t.mode][e] ? M[t.mode][e](t) : e === "rgb" ? M[t.mode].rgb(t) : M.rgb[e](M[t.mode].rgb(t)), M = {}, Ae = {}, je = [], Me = {}, Ne = (e) => e, N = (e) => (M[e.mode] = {
	...M[e.mode],
	...e.toMode
	...
	typeof e.interpolate[t] == "function" && (e.interpolate[t] = { use: e.interpolate[t] }), e.interpolate[t].fixup || (e.interpolate[t].fixup = Ne);
}), Ae[e.mode] = e, (e.parse || []).forEach((t) => {
	Fe(t, e.mode);
}), ke(e.mode)), Pe = (e) => Ae[e], Fe = (e, t) => {
	if (typeof e == "string") {
		if (!t) throw Error("'mode' required when 'parser' is a string");
		Me[e] = t;
	} else typeof e == "function" && je.indexOf(e) < 0 && je.push(e);
}, Ie = /[^\x00-\x7F]|[a-zA-Z_]/, Le = /[^\x00-\x7F]|[-\w]/, P = {
	Function: "function",
	Ident: "ident",
	Number: "number",
...
function Re(e) {
	let t = e[F], n = e[F + 1];
	return t === "-" || t === "+" ? /\d/.test(n) || n === "." && /\d/.test(e[F + 2]) : t === "." ? /\d/.test(n) : /\d/.test(t);
}
function ze(e) {
	if (F >= e.length) return !1;
	let t = e[F];
	if (Ie.test(t)) return !0;
	if (t === "-") {
...
function Ve(e) {
	let t = "";
	if ((e[F] === "-" || e[F] === "+") && (t += e[F++]), t += He(e), e[F] === "." && /\d/.test(e[F + 1]) && (t += e[F++] + He(e)), (e[F] === "e" || e[F] === "E") && ((e[F + 1] === "-" || e[F + 1] === "+") && /\d/.test(e[F + 2]) ? t += e[F++] + e[F++] + He(e) : /\d/.test(e[F + 1]) && (t += e[F++] + He(e))), ze(e)) {
		let n = Ue(e);
...
function Ue(e) {
	let t = "";
	for (; F < e.length && Le.test(e[F]);) t += e[F++];
	return t;
}
function We(e) {
	let t = Ue(e);
...
function Ke(e) {
	e._i = 0;
	let t = e[e._i++];
	if (!t || t.type !== P.Function || t.value !== "color" || (t = e[e._i++], t.type !== P.Ident)) return;
	let n = Me[t.value];
	if (!n) return;
	let r = { mode: n }, i = qe(e, !1);
	if (!i) return;
	let a = Pe(n).channels;
	for (let e = 0, t, n; e < a.length; e++) t = i[e], n = a[e], t.type !== P.None && (r[n] = t.type === P.Number ? t.value : t.value / 100, n === "alpha" && (r[n] = Math.max(0, Math.min(1, r[n]))));
	return r;
}
function qe(e, t) {
	let n = [], r;
	for (; e._i < e.length;) {
		if (r = e[e._i++], r.type === P.None || r.type === P.Number || r.type === P.Alpha || r.type === P.Percentage || t && r.type === P.Hue) {
			n.push(r);
			continue;
		}
	...
	if (!(n.length < 3 || n.length > 4)) {
		if (n.length === 4) {
			if (n[3].type !== P.Alpha) return;
			n[3] = n[3].value;
		}
		return n.length === 3 && n.push({
			type: P.None,
			value: void 0
		}), n.every((e) => e.type !== P.Alpha) ? n : void 0;
	}
}
function Je(e, t) {
	e._i = 0;
	let n = e[e._i++];
	if (!n || n.type !== P.Function) return;
	let r = qe(e, t);
	if (r) return r.unshift(n.value), r;
}
var Ye = (e) => {
	if (typeof e != "string") return;
	let t = Ge(e), n = t ? Je(t, !0) : void 0, r, i = 0, a = je.length;
	for (; i < a;) if ((r = je[i++](e, n)) !== void 0) return r;
...
function Xe(e, t) {
	...
	if (!(r.type === P.Hue || i.type === P.Hue || a.type === P.Hue)) return r.type !== P.None && (n.r = r.type === P.Number ? r.value / 255 : r.value / 100), i.type !== P.None && (n.g = i.type === P.Number ? i.value / 255 : i.value / 100), a.type !== P.None && (n.b = a.type === P.Number ? a.value / 255 : a.value / 100), o.type !== P.None && (n.alpha = Math.min(1, Math.max(0, o.type === P.Number ? o.value : o.value / 100))), n;
}
//#endregion
//#region ../../../node_modules/culori/src/rgb/parseTransparent.js
var Ze = (e) => e === "transparent" ? {
	mode: "rgb",
	r: 0,
	g: 0,
	b: 0,
	alpha: 0
} : void 0, Qe = (e, t, n) => e + n * (t - e), $e = (e) => {
	let t = [];
...
}, et = (e = 0) => Math.abs(e) ** (563 / 256) * Math.sign(e), tt = (e) => {
	let t = et(e.r), n = et(e.g), r = et(e.b), i = {
		mode: "xyz65",
...
}, nt = (e) => Math.abs(e) ** (256 / 563) * Math.sign(e), rt = ({ x: e, y: t, z: n, alpha: r }) => {
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = {
		mode: "a98",
		r: nt(e * 2.0415879038107465 - t * .5650069742788597 - .3447313507783297 * n),
		g: nt(e * -.9692436362808798 + t * 1.8759675015077206 + .0415550574071756 * n),
		b: nt(e * .0134442806320312 - t * .1183623922310184 + 1.0151749943912058 * n)
	};
	return r !== void 0 && (i.alpha = r), i;
}, it = (e = 0) => {
	let t = Math.abs(e);
	return t <= .04045 ? e / 12.92 : (Math.sign(e) || 1) * ((t + .055) / 1.055) ** 2.4;
}, z = ({ r: e, g: t, b: n, alpha: r }) => {
	let i = {
		mode: "lrgb",
		r: it(e),
		g: it(t),
...
}, U = (e) => (e %= 360) < 0 ? e + 360 : e, st = (e, t) => e.map((n, r, i) => {
	if (n === void 0) return n;
	let a = U(n);
	return r === 0 || e[r - 1] === void 0 ? a : t(a - U(i[r - 1]));
}).reduce((e, t) => !e.length || t === void 0 || e[e.length - 1] === void 0 ? (e.push(t), e) : (e.push(t + e[e.length - 1]), e), []), W = (e) => st(e, (e) => Math.abs(e) <= 180 ? e : e - 360 * Math.sign(e)), G = [
	-.14861,
	1.78277,
	-.29227,
	-.90649,
	1.97294,
	0
], ct = Math.PI / 180, lt = 180 / Math.PI, ut = G[3] * G[4], dt = G[1] * G[4], ft = G[1] * G[2] - G[0] * G[3], pt = ({ r: e, g: t, b: n, alpha: r }) => {
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = (ft * n + e * ut - t * dt) / (ft + ut - dt), a = n - i, o = (G[4] * (t - i) - G[2] * a) / G[3], s = {
		mode: "cubehelix",
		l: i,
		s: i === 0 || i === 1 ? void 0 : Math.sqrt(a * a + o * o) / (G[4] * i * (1 - i))
	};
	return s.s && (s.h = Math.atan2(o, a) * lt - 120), r !== void 0 && (s.alpha = r), s;
}, mt = ({ h: e, s: t, l: n, alpha: r }) => {
	let i = { mode: "rgb" };
	e = (e === void 0 ? 0 : e + 120) * ct, n === void 0 && (n = 0);
	let a = t === void 0 ? 0 : t * n * (1 - n), o = Math.cos(e), s = Math.sin(e);
	return i.r = n + a * (G[0] * o + G[1] * s), i.g = n + a * (G[2] * o + G[3] * s), i.b = n + a * (G[4] * o + G[5] * s), r !== void 0 && (i.alpha = r), i;
}, ht = (e, t) => {
	if (e.h === void 0 || t.h === void 0 || !e.s || !t.s) return 0;
	let n = U(e.h), r = U(t.h), i = Math.sin((r - n + 360) / 2 * Math.PI / 180);
	...
	return a && (o.h = U(Math.atan2(n, t) * 180 / Math.PI)), r !== void 0 && (o.alpha = r), o;
	...
	Y: 1,
	Z: .3583 / .329
}, St = (e) => e ** 3 > bt ? e ** 3 : (116 * e - 16) / yt, Ct = ({ l: e, a: t, b: n, alpha: r }) => {
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
...
}, wt = (e) => H(Ct(e)), Tt = (e) => e > bt ? Math.cbrt(e) : (yt * e + 16) / 116, Et = ({ x: e, y: t, z: n, alpha: r }) => {
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = Tt(e / xt.X), a = Tt(t / xt.Y), o = Tt(n / xt.Z), s = {
		mode: "lab65",
		l: 116 * a - 16,
		a: 500 * (i - a),
		b: 200 * (a - o)
	...
	return s.c && (s.h = U((Math.atan2(a, i) + Ot) / Math.PI * 180)), r !== void 0 && (s.alpha = r), s;
}, Pt = (e) => Mt(q(e, "dlch")), Ft = (e) => J(Nt(e), "dlab"), It = {
	mode: "dlab",
	parse: ["--din99o-lab"],
...
function Rt({ h: e, s: t, i: n, alpha: r }) {
	e = U(e === void 0 ? 0 : e), t === void 0 && (t = 0), n === void 0 && (n = 0);
...
}, Wt = RegExp(`^hsla?\\(\\s*${Ce}${we}${be}${we}${be}\\s*(?:,\\s*${xe}\\s*)?\\)$`), Gt = (e) => {
	let t = e.match(Wt);
	if (!t) return;
	let n = { mode: "hsl" };
...
function Kt(e, t) {
	...
	if (r.type !== P.None) {
		if (r.type === P.Percentage) return;
		n.h = r.value;
	}
	if (i.type !== P.None) {
		if (i.type === P.Hue) return;
		n.s = i.value / 100;
	}
	if (a.type !== P.None) {
	...
	return o.type !== P.None && (n.alpha = Math.min(1, Math.max(0, o.type === P.Number ? o.value : o.value / 100))), n;
}
//#endregion
...
var ln = (e) => Math.max(e / 203, 0), un = ({ i: e, t, p: n, alpha: r }) => {
	...
	return i && (a.h = U(Math.atan2(n, t) * 180 / Math.PI)), r !== void 0 && (a.alpha = r), a;
	...
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = Ln(e / Y.X), a = Ln(t / Y.Y), o = Ln(n / Y.Z), s = {
		mode: "lab",
		l: 116 * a - 16,
		a: 500 * (i - a),
		b: 200 * (a - o)
...
function Bn(e, t) {
	...
	if (!(r.type === P.Hue || i.type === P.Hue || a.type === P.Hue)) return r.type !== P.None && (n.l = Math.min(Math.max(0, r.value), 100)), i.type !== P.None && (n.a = i.type === P.Number ? i.value : i.value * 125 / 100), a.type !== P.None && (n.b = a.type === P.Number ? a.value : a.value * 125 / 100), o.type !== P.None && (n.alpha = Math.min(1, Math.max(0, o.type === P.Number ? o.value : o.value / 100))), n;
}
//#endregion
...
function Un(e, t) {
	...
	if (r.type !== P.None) {
		if (r.type === P.Hue) return;
		n.l = Math.min(Math.max(0, r.value), 100);
	}
	if (i.type !== P.None && (n.c = Math.max(0, i.type === P.Number ? i.value : i.value * 150 / 100)), a.type !== P.None) {
...
var Wn = {
	...
	return r !== void 0 && (i.alpha = r), i;
}, Jn = (e, t, n) => 4 * e / (e + 15 * t + 3 * n), Yn = (e, t, n) => 9 * t / (e + 15 * t + 3 * n), Xn = Jn(Y.X, Y.Y, Y.Z), Zn = Yn(Y.X, Y.Y, Y.Z), Qn = (e) => e <= jn ? An * e : 116 * Math.cbrt(e) - 16, $n = ({ x: e, y: t, z: n, alpha: r }) => {
	e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = Qn(t / Y.Y), a = Jn(e, t, n), o = Yn(e, t, n);
	...
	return r !== void 0 && (s.alpha = r), s;
}, er = (e, t, n) => 4 * e / (e + 15 * t + 3 * n), tr = (e, t, n) => 9 * t / (e + 15 * t + 3 * n), nr = er(Y.X, Y.Y, Y.Z), rr = tr(Y.X, Y.Y, Y.Z), ir = ({ l: e, u: t, v: n, alpha: r }) => {
	...
	t === void 0 && (t = 0), n === void 0 && (n = 0);
	let i = t / (13 * e) + nr, a = n / (13 * e) + rr, o = Y.Y * (e <= 8 ? e / An : ((e + 16) / 116) ** 3), s = {
		mode: "xyz50",
		x: 9 * i * o / (4 * a),
		y: o,
		z: o * (12 - 3 * i - 20 * a) / (4 * a)
...
function gr(e, t, n, r, i, a = null) {
	...
	if ((n - i) * a[1] - (a[0] - i) * r <= 0) o = a[1] * i / (r * a[0] + a[1] * (i - n));
		...
		o = a[1] * (i - 1) / (r * (a[0] - 1) + a[1] * (i - n));
		{
			...
			{
				let e = i * (1 - o) + o * n, t = o * r, a = e + t * c, s = e + t * l, m = e + t * u, h = a * a * a, g = s * s * s, _ = m * m * m, v = 3 * d * a * a, y = 3 * f * s * s, b = 3 * p * m * m, x = 6 * d * d * a, S = 6 * f * f * s, C = 6 * p * p * m, ee = 4.0767416621 * h - 3.3077115913 * g + .2309699292 * _ - 1, w = 4.0767416621 * v - 3.3077115913 * y + .2309699292 * b, T = 4.0767416621 * x - 3.3077115913 * S + .2309699292 * C, E = w / (w * w - .5 * ee * T), te = -ee * E, ne = -1.2684380046 * h + 2.6097574011 * g - .3413193965 * _ - 1, D = -1.2684380046 * v + 2.6097574011 * y - .3413193965 * b, re = -1.2684380046 * x + 2.6097574011 * S - .3413193965 * C, ie = D / (D * D - .5 * ne * re), O = -ne * ie, k = -.0041960863 * h - .7034186147 * g + 1.707614701 * _ - 1, ae = -.0041960863 * v - .7034186147 * y + 1.707614701 * b, oe = -.0041960863 * x - .7034186147 * S + 1.707614701 * C, A = ae / (ae * ae - .5 * k * oe), se = -k * A;
				te = E >= 0 ? te : 1e6, O = ie >= 0 ? O : 1e6, se = A >= 0 ? se : 1e6, o += Math.min(te, Math.min(O, se));
			}
		}
	}
	return o;
...
function yr(e) {
	...
	return l && (i.s = l, i.h = U(Math.atan2(r, n) * 180 / Math.PI)), i;
}
//#endregion
...
function Sr(e) {
	...
	return v.s && (v.h = U(Math.atan2(r, n) * 180 / Math.PI)), e.alpha !== void 0 && (v.alpha = e.alpha), v;
}
//#endregion
...
function Tr(e, t) {
	...
	if (!(r.type === P.Hue || i.type === P.Hue || a.type === P.Hue)) return r.type !== P.None && (n.l = Math.min(Math.max(0, r.type === P.Number ? r.value : r.value / 100), 1)), i.type !== P.None && (n.a = i.type === P.Number ? i.value : i.value * .4 / 100), a.type !== P.None && (n.b = a.type === P.Number ? a.value : a.value * .4 / 100), o.type !== P.None && (n.alpha = Math.min(1, Math.max(0, o.type === P.Number ? o.value : o.value / 100))), n;
}
//#endregion
...
function Dr(e, t) {
	...
	if (r.type !== P.None) {
		if (r.type === P.Hue) return;
		n.l = Math.min(Math.max(0, r.type === P.Number ? r.value : r.value / 100), 1);
	}
	if (i.type !== P.None && (n.c = Math.max(0, i.type === P.Number ? i.value : i.value * .4 / 100)), a.type !== P.None) {
		if (a.type === P.Percentage) return;
		n.h = a.value;
	}
	return o.type !== P.None && (n.alpha = Math.min(1, Math.max(0, o.type === P.Number ? o.value : o.value / 100))), n;
...
var Or = {
	...
	parse: ["rec2020"],
	serialize: "rec2020"
}, X = .0037930732552754493, Kr = Math.cbrt(X), qr = (e) => Math.cbrt(e) - Kr, Jr = (e) => {
	let { r: t, g: n, b: r, alpha: i } = z(e), a = qr(.3 * t + .622 * n + .078 * r + X), o = qr(.23 * t + .692 * n + .078 * r + X), s = qr(.2434226892454782 * t + .2047674442449682 * n + .5518098665095535 * r + X), c = {
		mode: "xyb",
		x: (a - o) / 2,
		y: (a + o) / 2,
		b: s - (a + o) / 2
	...
	toMode: { rgb: ({ x: e, y: t, b: n, alpha: r }) => {
		e === void 0 && (e = 0), t === void 0 && (t = 0), n === void 0 && (n = 0);
		let i = Yr(e + t) - X, a = Yr(t - e) - X, o = Yr(n + t) - X, s = V({
...
N(ot), N(vt), N(It), N(Lt), N(Bt), N(qt), N(Xt), N(en), N(pn), N(En), N(kn), N(Vn), N(Hn), N(Wn), N(Gn), N(ar), N(or), N(sr), N(xr), N(wr), N(Er), N(Or), N(jr), N(Ir), N(Gr), N(R), N(Xr), N(Zr), N(Qr), N($r);
//#endregion
...
function bi(e) {
	...
	return n && (t = String(n[1] ?? "").trim().toLowerCase()), t = t.replace(/\s+/g, ""), t ? t === "viewer" || yi.has(t) ? vi : t : "";
}
//#endregion
...
var xi = [
...
], Si = "cw::workspace::speed-dial", Ci = `${Si}::meta`, wi = (e) => {
	if (typeof structuredClone == "function") try {
		return structuredClone(x(e));
	} catch {}
	try {
		return JSON.parse(JSON.stringify(x(e)));
	} catch {
		return e;
	}
}, Ti = () => typeof crypto < "u" && typeof crypto.randomUUID == "function" ? crypto.randomUUID() : `sd-${Date.now().toString(36)}-${Math.floor(Math.random() * 1e3)}`, Ei = [
	{
		id: "shortcut-docs",
		cell: v([0, 1]),
		icon: "book-open-text",
...
})(Di), Ai = [], ji = (e) => e && Array.isArray(e) && e.length >= 2 ? v([Number(e[0]) || 0, Number(e[1]) || 0]) : v([0, 0]), Mi = (e = {}) => O(v({
	...
	for (let [n, r] of e) t.set(n, Mi(r));
	return t;
}, Pi = (e) => e ? e instanceof Map ? Array.from(e.entries()) : Array.isArray(e) ? e.map((e) => e && typeof e == "object" && "id" in e ? [e.id, e.meta || e] : null).filter(Boolean) : typeof e == "object" ? Object.entries(e) : [] : [], Fi = (e) => {
	let t = {};
	...
	let t = Pi(e);
	return Ni(t.length ? t : ki);
}, Ri = (e, t) => e && typeof e == "object" && "value" in e ? e.value ?? t : e ?? t, zi = (e) => ({
	id: e.id,
	cell: v([e.cell?.[0] ?? 0, e.cell?.[1] ?? 0]),
	icon: Ri(e.icon, "sparkle"),
	label: Ri(e.label, "Shortcut"),
	action: e.action
}), Bi = (e) => v({
	id: e.id || Ti(),
	cell: v(ji(e.cell)),
	icon: T(e.icon || "sparkle"),
	label: T(e.label || "Shortcut"),
	action: e.action || "open-view"
}), Vi = () => v(Oi.map(Bi)), Hi = (e) => v((Array.isArray(e) && e.length ? e : Di).map((e) => {
	let { meta: t, ...n } = e;
	return t ? Ai.push([e.id, {
		action: e.action,
		...t
	}]) : Ai.push([e.id, { action: e.action }]), n;
}).map(Bi)), Ui = (e) => e.map(zi), Wi = a(Ci, Ii, Li, Fi), Gi = a(Si, Vi, Hi, Ui), Ki = () => Gi?.$save?.(), qi = () => Wi?.$save?.(), Ji = (e) => e ? Wi?.get?.(e) ?? null : null, Yi = (e, t = {}) => {
	let n = Wi?.get?.(e);
	return n || (n = Mi(t), Wi?.set?.(e, n), qi()), t?.action && n.action !== t.action && (n.action = t.action), n;
}, Xi = (e) => {
	if (!e) return !1;
	let t = e.action || "open-view", n = Yi(e.id, { action: t });
	return n.action === t ? !1 : (n.action = t, !0);
};
Ai.length && (Ai.forEach(([e, t]) => {
	let n = Yi(e, t);
	Object.assign(n, t);
}), Ai.length = 0, qi()), (() => {
	let e = !1;
	...
	Ei.forEach((t) => {
		if (Gi?.find?.((e) => e?.id === t.id)) {
			...
			} else t.meta && !n && (Yi(t.id, t.meta), e = !0);
		} else {
			let n = Bi(t);
			t.label && n.label && typeof n.label == "object" && "value" in n.label && (n.label.value = t.label), t.icon && n.icon && typeof n.icon == "object" && "value" in n.icon && (n.icon.value = t.icon), Gi.push(v(n)), Yi(n.id, t.meta), e = !0;
		}
	}), e && (Ki(), qi());
})(), a("cw::workspace::wallpaper", () => v({
...
var Zi = a("cw::workspace::grid-layout", () => v({
	columns: 4,
	rows: 8,
	shape: "square"
}), (e) => v(e || {
	...
	let t = e?.grid || Zi, n = t?.columns ?? 4, r = t?.rows ?? 8, i = t?.shape ?? "square";
	Zi && (Zi.columns = n, Zi.rows = r, Zi.shape = i, Qi()), !(typeof document > "u") && (document.querySelectorAll(".speed-dial-grid").forEach((e) => {
		let t = e;
...
function ea(e) {
	let t = String(e ?? "").trim();
	if (!t) return "";
	let n = t.toLowerCase().replace(/^#/, ""), r = xi.find((e) => String(e.view).toLowerCase() === n || String(e.label).trim().toLowerCase() === n);
	return r ? String(r.view) : bi(t) || t.replace(/^#/, "").trim();
}
//#endregion
//#region ../home-view/src/ts/OrientDesktop.ts
var ta = null, na = () => {
	ta ||= m(me);
}, ra = 280, ia = "cw-speed-dial-item", aa = "cw-speed-dial-registry", oa = /(https?:\/\/[^\s<>"']+)/i, sa = [{
	value: "open-view",
	label: "Open view"
}, {
	value: "open-link",
...
}, ca = (e) => e.length ? new Set(e.map((e) => Z(e.shape))).size === 1 ? Z(e[0].shape) : "mixed" : "squircle", Q = {
	columns: 4,
	rows: 8,
	items: [
		{
...
}, la = new Set(Q.items.map((e) => e.id)), ua = (e = "item") => typeof crypto < "u" && typeof crypto.randomUUID == "function" ? crypto.randomUUID() : `${e}-${Date.now().toString(36)}-${Math.floor(Math.random() * 1e4)}`, da = (e, t, n) => [Math.max(0, Math.min(t - 1, Math.round(e[0]))), Math.max(0, Math.min(n - 1, Math.round(e[1])))], fa = (e) => `${e[0]}:${e[1]}`, pa = (e, t, n, r) => {
	let i = da(e, n, r);
	if (!t.has(fa(i))) return i;
	let a = Math.max(n, r);
	for (let e = 1; e <= a; e += 1) for (let a = Math.max(0, i[1] - e); a <= Math.min(r - 1, i[1] + e); a += 1) for (let r = Math.max(0, i[0] - e); r <= Math.min(n - 1, i[0] + e); r += 1) {
		...
		let n = [r, a];
		if (!t.has(fa(n))) return n;
	}
	return i;
}, ma = (e, t, n) => {
	let r = /* @__PURE__ */ new Set();
	for (let i of e) {
		let e = pa(i.cell, r, t, n);
		i.cell = e, r.add(fa(e));
	}
	return e;
}, ha = (e, t, n) => {
	...
	let i = String(e?.action || (e?.href ? "open-link" : "open-view")), a = {
		...
		iconSrc: C(e?.iconSrc, e?.href, i),
		viewId: String(e?.viewId || "home"),
		cell: da([Number(e?.cell?.[0] || 0), Number(e?.cell?.[1] || 0)], t, n),
	...
	try {
		...
		let n = Math.max(4, Math.min(8, Number(t.columns || Q.columns))), r = Math.max(6, Math.min(12, Number(t.rows || Q.rows))), i = [...Q.items], a = ma((Array.isArray(t.items) && t.items.length ? t.items : i).map((e) => ha(e, n, r)).filter((e) => !!e), n, r);
		return {
			columns: n,
			rows: r,
			items: a.length ? a : ma(i.map((e) => ha({
				...e,
	...
	return null;
}, va = (e) => Array.from(e.dataTransfer?.files || []).find((e) => e.type?.startsWith("image/")) || null, ya = (e) => new Promise((t, n) => {
	...
	let n = xa(e);
	return n ? Ca(n, t) : null;
}, Ta = (e, t, n, r) => {
	if (!e) return [];
	let i = e;
	return (Array.isArray(i?.items) ? i.items : Array.isArray(e) ? e : i?.item ? [i.item] : [e]).map((e, i) => ha({
		...e || {},
		id: String(e?.id || ua("import")),
	...
	if (a.startsWith("{") || a.startsWith("[")) try {
		...
		if (e?.k === "cw-sdi") {
			let t = y(e);
			if (t?.id) return Ta({ items: [t] }, n, r, i);
		}
		if (e?.kind === ia || e?.kind === aa || e?.items || e?.item || Array.isArray(e)) return Ta(e, n, r, i);
	} catch {}
	let s = Sa(o);
	if (s) return [Ca(s, i, (() => {
		try {
	...
	...e,
	iconSrc: u(e.iconSrc || "", e.action, e.href)
})), Oa = (e) => JSON.stringify({
	kind: aa,
	version: 1,
	columns: e.columns,
	...
	console.warn("[OrientDesktop] No view opener registered; using hash fallback for:", t), g(`#${t}`);
}, ja = (e) => {
	let t = String(e || "").trim();
	...
	t.dataset.desktopMounted = "true", na();
	let r = ga(), a = new Map(r.items.map((e) => [e.id, e])), o = r.items.map((e) => e.id), s = null, c = document.createElement("div");
	...
	f.className = "speed-dial-grid speed-dial-grid--labels ui-launcher-grid app-oriented-desktop__grid app-oriented-desktop__grid--labels", f.dataset.gridLayer = "icons", f.setAttribute("data-grid-columns", String(r.columns)), f.setAttribute("data-grid-rows", String(r.rows)), u(f), f.dataset.dialStack = "shapes";
	let m = document.createElement("div");
	m.className = "speed-dial-grid speed-dial-grid--icons ui-launcher-grid app-oriented-desktop__grid app-oriented-desktop__grid--icons", m.dataset.gridLayer = "labels", m.setAttribute("data-grid-columns", String(r.columns)), m.setAttribute("data-grid-rows", String(r.rows)), u(m), m.dataset.dialStack = "text", c.append(f, m), t.appendChild(c);
	let h = () => {
		let e = ca(r.items);
		f.setAttribute("data-grid-shape", e), m.setAttribute("data-grid-shape", e);
	};
	h();
	let g = () => {
		s !== null && (clearTimeout(s), s = null), ee(r.columns, r.rows, Da(r.items)), h();
	}, v = () => {
		s !== null && clearTimeout(s), s = setTimeout(() => {
			s = null, _(r.columns, r.rows, Da(r.items));
		}, 400);
		...
		for (let n of r.items) e && n.id === e || t.add(fa(n.cell));
		return t;
	}, w = (e, t) => {
		e.cell = da(t, r.columns, r.rows);
		let n = b.get(e.id), i = x.get(e.id);
		n && $(n, e.cell), i && $(i, e.cell);
	}, T = (e, t, n = "") => {
		let i = pa(t, C(n), r.columns, r.rows);
		return w(e, i), i;
	}, E = (e, t) => {
		let n = 0;
		for (let i = 0; i < e.length; i += 1) {
			...
			!c || a.has(c.id) || (c.cell = pa(c.cell, C(), r.columns, r.rows), r.items.push(c), a.set(c.id, c), o.push(c.id), A(c), n += 1);
		}
		return n > 0 && g(), n;
	}, ne = (t) => {
		...
		if (n) {
			...
			if (r) {
				...
				if (a) if (i?.remove(), n) n.src = a, n.alt = t.label ? String(t.label) : "";
				else {
					let e = document.createElement("img");
					e.className = "ui-ws-item-icon-image", e.alt = t.label ? String(t.label) : "", e.loading = "lazy", e.decoding = "async", e.referrerPolicy = "no-referrer", e.src = a, e.addEventListener("error", () => e.remove()), r.insertBefore(e, r.firstChild);
				}
				else n && n.remove(), i || (i = document.createElement("ui-icon"), i.setAttribute("icon-style", "duotone"), r.appendChild(i)), i.setAttribute("icon-style", "duotone"), i.setAttribute("icon", t.icon || "sparkle");
			}
			$(n, t.cell);
		}
	}, D = (e, t) => d(f, [e, t], {
			...
			if (navigator.clipboard?.read) {
				...
				for (let n of t) {
					if (n.types.includes("image/png") || n.types.includes("image/jpeg") || n.types.includes("image/webp")) {
						...
						if (await ba(new File([t], "wallpaper", { type: t.type }))) return !0;
					}
					let t = n.types.includes("text/plain") ? "text/plain" : "", i = n.types.includes("text/html") ? "text/html" : "", a = Ea(t ? await (await n.getType(t)).text() : "", i ? await (await n.getType(i)).text() : "", r.columns, r.rows, e);
					if (a.length) return E(a, e) > 0;
				}
			}
			return E(Ea(await navigator.clipboard.readText(), "", r.columns, r.rows, e), e) > 0;
		} catch {
			return !1;
		}
	}, ie = (t) => {
		...
		if (i) {
			let e = document.createElement("img");
			e.className = "ui-ws-item-icon-image", e.alt = t.label ? String(t.label) : "", e.loading = "lazy", e.decoding = "async", e.referrerPolicy = "no-referrer", e.src = i, e.addEventListener("error", () => e.remove()), r.appendChild(e);
		} else {
			let e = document.createElement("ui-icon");
			e.setAttribute("icon-style", "duotone"), e.setAttribute("icon", t.icon || "sparkle"), r.appendChild(e);
		}
		...
		let n = o.indexOf(e);
		n >= 0 && o.splice(n, 1), b.get(e)?.remove(), x.get(e)?.remove(), b.delete(e), x.delete(e), ma(r.items, r.columns, r.rows), g();
	}, A = (e) => {
		let t = ie(e), n = O(e);
		b.set(e.id, t), x.set(e.id, n), f.appendChild(t), m.appendChild(n);
		let i = t.querySelector(".ui-ws-item-icon");
		i && (i.style.pointerEvents = "auto", i.style.touchAction = "none"), le(t, {
			...
			v(), t.dataset.interactionState = "onMoving", t.dataset.gridCoordinateState = "intermediate";
		}), t.addEventListener("m-dragend", () => {
			y = performance.now() + ra, t.dataset.interactionState = "onRelax", t.dataset.gridCoordinateState = "destination";
			let n = x.get(e.id);
			...
			t.preventDefault(), t.stopPropagation(), !(performance.now() < y) && Aa(e);
		});
	}, se = async (e) => re(e), ce = (e, t) => {
		let n = !e, r = t?.seed || {}, i = t?.suggestedCell || [0, 0], o = e || {
			id: ua("item"),
			label: r.label || "New shortcut",
			icon: r.icon || "sparkle",
			iconSrc: "",
			viewId: Ma(r.viewId, vi),
			cell: i,
		...
		pe({
			...
			onSave: (e) => {
				...
				if (o.label = String(e.label || "Item").trim() || "Item", o.icon = String(e.icon || "sparkle").trim() || "sparkle", o.action = t, o.href = t === "open-link" ? r : "", o.viewId = t === "open-link" ? "home" : Ma(e.view, vi), o.shape = Z(e.shape), t === "open-link" && r) try {
				...
				if (n) E([o], i);
					...
					let e = a.get(o.id);
					e && (Object.assign(e, o), a.set(e.id, e), ne(e), g());
				}
			},
			onDelete: n ? void 0 : () => {
				k(o.id);
		...
		let i = t ? [
			...
			},
			{
				...
				children: [...t.action === "open-link" && t.href ? [{
					...
					action: async () => {
						try {
							await navigator.clipboard.writeText(p(t));
						} catch {}
					}
				}]
			},
			{
				...
				children: [{
					...
					action: () => ce(t, { suggestedCell: t.cell })
				}, {
					id: "remove",
					label: "Remove",
					icon: "trash",
					danger: !0,
					disabled: la.has(t.id),
					action: () => k(t.id)
				}]
			}
		] : [
				...
				children: [
					{
						...
						icon: "plus",
						action: () => ce(void 0, { suggestedCell: n })
					},
					{
						id: "paste-link",
				...
				children: [
					{
						...
						action: async () => {
							try {
								await navigator.clipboard.writeText(Oa(r));
							} catch {}
						}
					},
					{
						...
						action: () => {
							...
							ka(`cw-home-registry-${`${e.getFullYear()}-${String(e.getMonth() + 1).padStart(2, "0")}-${String(e.getDate()).padStart(2, "0")}`}.json`, Oa(r));
						}
					},
					{
		...
		ae({
			...
			resolveOverlayMountPoint: _i() ?? void 0
		});
	}, de = async (e) => {
		let t = _a(e);
		if (t) {
			e.preventDefault(), e.stopPropagation(), await ba(t);
			return;
		}
		let n = Ea(e.clipboardData?.getData("text/plain") || "", e.clipboardData?.getData("text/html") || "", r.columns, r.rows, [0, 0]);
	...
	c.addEventListener("pointerdown", () => c.focus()), c.addEventListener("dragover", (e) => {
		...
		let n = e.dataTransfer?.getData("text/plain") || "", i = e.dataTransfer?.getData("text/html") || "", a = Ea([e.dataTransfer?.getData("text/uri-list") || "", n].filter(Boolean).join("\n").trim(), i, r.columns, r.rows, [0, 0]);
		if (!a.length) {
			let t = Array.from(e.dataTransfer?.files || []).find((e) => e.type === "text/plain" || e.type === "text/html");
			if (t) {
				let e = await t.text();
				a = Ea(e, t.type === "text/html" ? e : "", r.columns, r.rows, [0, 0]);
			}
		}
		...
		ue(e, t && a.get(t) || null, D(e.clientX, e.clientY));
	});
	for (let e of r.items) A(e);
}, Fa = class {
	id = "home";
	name = "Home";
	icon = "house";
	...
	element = null;
	lifecycle = { onUnmount: () => {
		pi(null), gi(null), this.element = null;
	} };
	constructor(e = {}) {
	...
	render(e) {
		...
		}), gi(typeof this.shellContext?.resolveOverlayMountPoint == "function" ? (e) => this.shellContext.resolveOverlayMountPoint(e) : null), Pa(t), this.element = t, t;
	}
	invokeChannelApi(e, t) {
		if (e !== k.Navigate && e !== k.OpenView) return;
...
var La = Ia;
//#endregion
export { Fa as HomeView, La as createHomeView, Ia as createView, Ia as default, Pa as initializeOrientedDesktop };
